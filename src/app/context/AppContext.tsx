...
export interface Alert {
  id: string;
  pair: string;
  type: 'rises_above' | 'falls_below' | 'percent_change';
  targetRate: number;
...
interface AppContextType {
  ...
  toggleAlertMonitoring: (id: string) => void;
  currencies: Currency[];
  recentConversions: RecentConversion[];
  addRecentConversion: (entry: RecentConversion) => void;
}
...
export const AppProvider = ({ children }: { children: ReactNode }) => {
  const [favorites, setFavorites] = useState<CurrencyPair[]>([
    { id: '1', from: 'USD', to: 'EUR', rate: 0.923 },
    { id: '2', from: 'USD', to: 'INR', rate: 95.306 },
    { id: '3', from: 'USD', to: 'GBP', rate: 0.74 },
    { id: '4', from: 'USD', to: 'JPY', rate: 156.57 },
  ]);
  ...
  const [alerts, setAlerts] = useState<Alert[]>([
    ...
    },
    {
      ...
      repeat: 'once',
      status: 'triggered',
      triggeredDate: 'Apr 30',
      monitoring: false,
    },
  ]);
  const [recentConversions, setRecentConversions] = useState<RecentConversion[]>([]);

  const addRecentConversion = (entry: RecentConversion) => {
    setRecentConversions(prev => {
      const deduped = prev.filter(c => !(c.from === entry.from && c.to === entry.to));
  ...
  const currencies: Currency[] = [
    ...
    { code: 'ANG', name: 'Netherlands Antillean Guilder', country: 'Caribbean Netherlands' },
    ...
    { code: 'BAM', name: 'Bosnia-Herzegovina Mark',       country: 'Bosnia & Herzegovina' },
    ...
    { code: 'MNT', name: 'Mongolian Togrog',              country: 'Mongolia' },
    ...
    { code: 'TOP', name: 'Tongan Paanga',                 country: 'Tonga' },
  ...
  const addFavorite = (pair: CurrencyPair) => {
    setFavorites((prev) => [...prev, { ...pair, id: Date.now().toString() }]);
  };
  ...
  const toggleAlertMonitoring = (id: string) => {
    setAlerts((prev) =>
      prev.map((alert) =>
        alert.id === id ? { ...alert, monitoring: !alert.monitoring } : alert
      )
    );
  };
